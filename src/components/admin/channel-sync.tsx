...
export const OAUTH_CONNECT_PATHS: Partial<Record<MarketplaceName, string>> = {
...
/**
 * The connection status card — shown on the Channels hub for every
 * marketplace. When connected and implemented, links through to that
 * marketplace's own dedicated management page (product sync, auto-connect,
 * activity log) rather than cramming everything onto one page.
 */
    ...
    <Card className="p-5">
      ...
      <div className="mt-4 flex flex-col gap-2">
        {!info.implemented ? (
          ...
          <>
            <Link
              to="/admin/channels/$marketplace"
              params={{ marketplace: info.marketplace }}
...
export const CONNECTION_FILTER_LABELS: Record<ConnectionFilter, string> = {
  all: 'All statuses',
  connected: 'Connected',
...
export function RecentActivity({ logs }: { logs: SyncLogRow[] }) {
  if (logs.length === 0) {
    return null
  }
  return (
