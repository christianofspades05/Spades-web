/**
 * Shopee's OAuth redirect target. Exchanges the `code`/`shop_id` params for
 * real tokens and upserts the one `marketplace_connections` row for Shopee
  ...
  server: {
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          const tokens = await adapter.exchangeCodeForTokens(code, shopId)
        ...
        return redirectTo(`${url.origin}/admin/channels?connected=shopee`)
      },
    },
  },
})
