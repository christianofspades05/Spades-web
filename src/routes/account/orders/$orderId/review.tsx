...
import {
  getOrderReviewProducts,
  submitAccountReviews,
  uploadAccountReviewPhoto,
...
export const Route = createFileRoute('/account/orders/$orderId/review')({
  beforeLoad: async () => {
    const customer = await getCustomerSession()
    if (!customer) throw redirect({ to: '/account/login' })
  },
  loader: ({ params }) =>
    getOrderReviewProducts({ data: { orderId: params.orderId } }),
  component: AccountOrderReviewPage,
})
...
function AccountOrderReviewPage() {
  ...
  async function handlePhotoSelect(
    ...
    try {
      ...
      const { url } = await uploadAccountReviewPhoto({
        data: {
          orderId,
          fileName: file.name,
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    setSubmitting(true)
    try {
      await submitAccountReviews({ data: { orderId, reviews } })
      setSubmitted(true)
    } catch (err) {
      setError(getErrorMessage(err))
    } finally {
